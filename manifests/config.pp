...
class kubernetes::config inherits ::kubernetes {
  ...
  create_resources('kubernetes::config::template', $kubeconfigs, $::kubernetes::kubeconfig_defaults)

  $service_configs = lookup('kubernetes::service_configs', { 'value_type' => Hash, 'merge' => 'deep', 'default_value' => $::kubernetes::service_configs })
  create_resources('kubernetes::config::template', $service_configs, $::kubernetes::service_configs_defaults)
  ...
  if $::kubernetes::install_cni_net {
    ...
    create_resources('kubernetes::config::template', $cni_configs, $::kubernetes::cni_config_defaults)
  }
